...
type SnapshotCacheWithCallbacks interface {
	...
	serverv3.Callbacks
	GenerateNewSnapshot(string, xds.Resources) error
}
...
func (s *snapshotCache) GenerateNewSnapshot(irKey string, resources xds.Resources) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func NewSnapshotCache(ads bool, logger logger.Logger) SnapshotCacheWithCallbacks {
	// Set up the nasty wrapper hack.
	return &snapshotCache{
		SnapshotCache:    cachev3.NewSnapshotCache(ads, &Hash, logger.Sugar()),
		log:              logger.Sugar(),
